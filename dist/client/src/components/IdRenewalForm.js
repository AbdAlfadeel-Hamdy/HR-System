...
export const IdRenewalForm = ({ queryFn, groupByHandler, renewalDateHandler, }) => {
    ...
    return (<Container component="main" className="h-full">
      ...
      <Box component="form" onSubmit={(e) => {
        ...
        <TextField margin="normal" required fullWidth size="small" select name="month" label="Month" id="month" value={formik.values.month} onChange={formik.handleChange} onBlur={formik.handleBlur} error={formik.touched.month && Boolean(formik.errors.month)} helperText={formik.touched.month && formik.errors.month} className="bg-white">
          {months.map((option) => (<MenuItem key={option.value} value={option.value}>
              {option.label}
            </MenuItem>))}
        </TextField>
        <TextField margin="normal" required fullWidth size="small" select name="year" label="Year" id="year" value={formik.values.year} onChange={formik.handleChange} onBlur={formik.handleBlur} error={formik.touched.year && Boolean(formik.errors.year)} helperText={formik.touched.year && formik.errors.year}>
          {[
