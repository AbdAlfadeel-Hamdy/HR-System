...
import { cancelledColumns, downloadCancelledPDF, } from "../utils/pdfCreators/cancelled";
const CancelledReport = () => {
    ...
    const modifiedData = data.cancelled.map((row) => {
        return {
            ...row,
            cancellationDate: new Date(row.cancellationDate).toLocaleDateString("en-uk"),
        };
    });
    ...
    return (<>
      ...
      <DownloadButton onClick={() => downloadCancelledPDF("Cancelled Report", cancelledColumns, modifiedData)}/>
    </>);
};
