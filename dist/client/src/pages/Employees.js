...
const Employees = () => {
    ...
    const { isFetching, data, error, refetch } = useQuery({
        queryKey: ["employees"],
        queryFn: async () => {
            const { data } = await customFetch.get(`/employees${idNumber ? `?idNumber=${idNumber}` : ""}`);
            return data;
        },
    });
    useEffect(() => {
    ...
    const modifiedData = data.employees.map((row) => {
        return {
            ...row,
    ...
    return (<>
      <div className="flex flex-col row-span-2">
        <div className="bg-black h-12 flex justify-center items-center overflow-y-scroll">
          <input type="text" className=" py-1 px-3 placeholder-gray-400 placeholder:text-sm caret-black outline-none rounded-full mt-2" placeholder="Search by ID" value={idNumber} onChange={(e) => setIdNumber(e.target.value)}/>
        </div>
